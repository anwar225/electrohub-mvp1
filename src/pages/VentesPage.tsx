...
export function VentesPage() {
  ...
  const filtered = useMemo(() => {
    return (factures ?? []).filter((f) => {
      if (f.type !== 'vente') return false; // Only show sales invoices
      if (statusFilter !== 'all' && f.status !== statusFilter) return false;
      if (search) {
        const q = search.toLowerCase();
        if (
          !f.numero.toLowerCase().includes(q) &&
          !((f as any).clientNom || '').toLowerCase().includes(q)
        )
          return false;
      }
      return true;
  ...
  const updateItem = (index: number, field: string, value: any) => {
    ...
    if (field === 'produitId' && value) {
      ...
      if (selectedProduct) {
        ...
        newItems[index].prixUnitaire = selectedProduct.prixVente;
      }
    }
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <p className="py-16 text-center text-sm text-muted-foreground">Aucune facture de vente trouvée</p>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {paged.map((f) => (
                  <TableRow key={f.id}>
                    ...
                    <TableCell>{(f as any).clientNom || '-'}</TableCell>
      ...
      <Dialog open={!!detail} onOpenChange={(o) => !o && setDetail(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>Détail facture vente</DialogTitle>
          ...
          {detail && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <Info label="Client" value={(detail as any).clientNom || '-'} />
      ...
      <Dialog open={showCreateForm} onOpenChange={(o) => !o && setShowCreateForm(false)}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Créer une facture vente</DialogTitle>
            <DialogDescription>Saisissez les informations de la facture de vente</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
